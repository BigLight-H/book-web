        ...
        <Col span="24" class="room-con-list">
          ...
          <Col :xs="20" :lg="8" class="type-con">
            <Col :xs="12" :lg="12" v-for="(item,index) in info" :key="index">
              <div class="type-con-div" @click="getTypeLists(item.Domain)">{{ item.Name }}</div>
            </Col>
          </Col>
          <Col :xs="2" :lg="8">&nbsp;</Col>
        </Col>
...
<script>
  ...
  import NavInput from "./NavInput";
    export default {
        ...
        created(){
          axios.get('/book/source/types?time='+new Date().getTime()).then((res) => {
            this.info = res.data
          }).catch((error) => {
            console.warn(error)
          });
        ...
        methods: {
          getTypeLists(domain) {
            axios.get('/book/source/type?link='+ domain +'&time='+new Date().getTime()).then((res) => {
