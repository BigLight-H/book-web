...
<script>
  ...
  export default {
    ...
    created(){
      ...
      let link = sessionStorage.getItem('book_pai_hang');
      if (link) {
        axios.get('/book/source/board/list?link='+ link +'&time='+new Date().getTime()).then((res) => {
    ...
    beforeDestroy() {
      ...
      sessionStorage.removeItem('book_pai_hang');
    },
    methods: {
      getTypeLists(domain) {
